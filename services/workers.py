        ...
        try:
            ...
            if item:
                try:
                    ...
                    time_since_last_write = current_time - last_write_times.get(
                        channel_name, 0
                    )
                    ...
                    if feeds:
                        ...
                        logger.info(
                            f"Syncing {len(feeds)} entries to ThingSpeak {channel_name} in {len(batches)} batches"
                        )
                        ...
                        for batch_idx, batch in enumerate(batches):
                            ...
                            logger.info(
                                f"Batch {batch_idx + 1}/{len(batches)} synced: {result.get('feeds_written', 0)} entries"
                            )
                ...
                if current_time - last_sync >= SYNC_INTERVAL_SECONDS:
                    ...
                    for channel_name in [
                        ...
                        logger.debug(
                            f"Scheduled periodic sync for {channel_name} (since entry_id {last_synced_entry_id})"
                        )
...
def start_background_workers():
    """Start background worker threads and return them."""
    ...
    return worker_thread, sync_worker_thread
