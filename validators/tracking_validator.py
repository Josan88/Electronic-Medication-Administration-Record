"""
Medication tracking data validation for Electronic Medication Administration Record (eMAR).

This module validates medication tracking information before it's sent to ThingSpeak.
"""
...
def validate_consume_date(consume_date: str) -> str:
    ...
    for date_format in date_formats:
        try:
            datetime.strptime(consume_date, date_format)
            valid_date = True
            break
        except ValueError:
            continue
...
def validate_time_slot(time_slot: str) -> str:
    ...
    time_slot = sanitize_string(time_slot, max_length=50)
    
    # Time slot can contain times (8AM, 14:00, 08:00), commas, spaces
...
def validate_tracking_data(data: Dict[str, Any], check_patient: bool = False) -> Dict[str, Any]:
    """
    Validate all medication tracking data fields.
    ...
    if not isinstance(data, dict):
        raise ValidationError("Tracking data must be a valid JSON object")
